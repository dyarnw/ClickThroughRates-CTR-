...
server <- function(input, output, session) {
  ...
  # Displays an explanation of the ANOVA results
  output$box_left <- renderText({
    ...
    if (as.numeric(p_value) > 0.05) {
      ...
      paste("Because p-value(", p_value, ") < 0.05, there is a significant difference for the left sidebar group and the other groups")
    }
  })
  ...
  output$box_center <- renderText({
    ...
    if (as.numeric(p_value) > 0.05) {
      ...
      paste("Because p-value(", p_value, ") < 0.05, there is a significant difference for the center page group and the other groups")
    }
  })
  ...
  output$box_right <- renderText({
    ...
    if (as.numeric(p_value) > 0.05) {
      ...
      paste("Because p-value(", p_value, ") < 0.05, there is a significant difference for the right sidebar group and the other groups")
    }
  })
  ...
  output$box_all <- renderText({
    ...
    if (as.numeric(p_value) > 0.05) {
      ...
      paste("Because p-value(", p_value, ") < 0.05, there are significant differences for all groups")
    }
  })
